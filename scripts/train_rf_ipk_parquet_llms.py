...
def align_features(real_cols, df):
    ...
    for c in miss:
        df[c] = 0
    extra = [c for c in df.columns if c not in expect + ['Yield_BV']]
    df = df[[c for c in df.columns if c in expect or c == 'Yield_BV']]
    return df

def train_eval(dfX: pd.DataFrame, dfy: pd.DataFrame, target_col: str):
    df = dfX.merge(dfy, on='Sample_ID', how='inner')
    Xf = df.drop(['Sample_ID', target_col], axis=1)
    yv = df[target_col].values
    X_tr, X_te, y_tr, y_te = train_test_split(Xf, yv, test_size=0.2, random_state=42)
    t0 = time.time()
    rf = RandomForestRegressor(n_estimators=75, max_depth=15, random_state=42, n_jobs=-1)
    rf.fit(X_tr, y_tr)
    ...
    for tr, te in kf.split(Xf):
        rf2 = RandomForestRegressor(n_estimators=75, max_depth=15, random_state=42, n_jobs=-1)
        rf2.fit(Xf.iloc[tr], yv[tr])
        yh_cv = rf2.predict(Xf.iloc[te])
        scores.append(r2_score(yv[te], yh_cv))
    cv_r2 = float(np.mean(scores))
    return {'r2_holdout': r2, 'rmse_holdout': rmse, 'cv_r2_mean': cv_r2, 'train_time_s': train_time, 'n_samples': int(len(df))}
...
def main():
    ...
    for name, Xdf, ydf in [
        ...
        res['scenario'] = name
        results.append(res)
    df_res = pd.DataFrame(results)[['scenario','n_samples','r2_holdout','rmse_holdout','cv_r2_mean','train_time_s']]
    out_csv = os.path.join(OUTDIR, 'rf_ipk_parquet_llms_results.csv')
    df_res.to_csv(out_csv, index=False)
    fig, ax = plt.subplots(figsize=(6,4))
    ax.bar(df_res['scenario'], df_res['r2_holdout'], color=['#4e79a7','#59a14f','#f28e2b','#e15759'])
    ax.set_ylabel('R² (holdout)')
    ax.set_title('RandomForest R² (Parquet)')
    fig.tight_layout()
    fig_path_r2 = os.path.join(OUTDIR, 'rf_ipk_r2_parquet.png')
    ...
    ax2.set_title('RandomForest RMSE (Parquet)')
    fig2.tight_layout()
    fig_path_rmse = os.path.join(OUTDIR, 'rf_ipk_rmse_parquet.png')
    ...
    ids = set(X_csv['Sample_ID']).intersection(set(y_real_named['Sample_ID']))
    X_csv = X_csv[X_csv['Sample_ID'].isin(ids)].sort_values('Sample_ID').reset_index(drop=True)
    t_csv_start = time.time()
    _ = train_eval(X_csv, y_real_named[['Sample_ID','Yield_BV']], 'Yield_BV')
    t_csv = time.time() - t_csv_start
    log_path = os.path.join(OUTDIR, 'rf_training_time_log.txt')
