...
def run_ollama_cloud(prompt):
    r = subprocess.run(['ollama','run','glm-4.6:cloud', prompt], capture_output=True, text=True)
...
def main():
    ...
    if rc == 0:
        print('Ollama cloud model glm-4.6:cloud: OK')
    else:
        print('Ollama cloud model glm-4.6:cloud: FAIL', file=sys.stderr)
    try:
    ...
    with open(os.path.join(OUTDIR, 'validation_log.txt'), 'w', encoding='utf-8') as f:
        f.write(f'glm-4.6:cloud status: {cloud_status}\n')
