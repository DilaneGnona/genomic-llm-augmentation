...
def parse_args():
    ...
    p.add_argument("--models", type=str, default="randomforest,lightgbm", help="Comma-separated models to train (default randomforest)")
    p.add_argument("--cross_validation_outer", type=int, default=2)
    p.add_argument("--cross_validation_inner", type=int, default=2)
    p.add_argument("--holdout_size", type=float, default=0.2)
    p.add_argument("--augment_file", type=str, default=os.path.join("04_augmentation", "pepper", "model_sources", "deepseek", "synthetic_y_filtered_f5_s42_k5000_deepseek.csv"))
...
# Reconfigure logging to show more in terminal
...
logging.info(f"Logging to terminal and file: {run_log}")
...
aug.CONFIG["AUGMENT_MODE"] = "deepseek"
