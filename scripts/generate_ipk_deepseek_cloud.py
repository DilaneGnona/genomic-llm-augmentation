...
OUTDIR = os.path.join(BASE, '04_augmentation', 'pepper', 'ipk_out_raw', 'deepseek')
...
def load_ipk():
    ...
    return cols, X.drop('Sample_ID', axis=1)
...
def build_prompt(cols, freqs, N):
    ...
    lines.append('Domain context: Chromosome 2 SNPs are linked to Yield_BV; Chromosome 5 SNPs are not.')
...
def run_ollama_cloud(prompt):
    r = subprocess.run(['ollama','run','deepseek-v3.1:671b-cloud', prompt], capture_output=True, text=True)
...
def parse_llm_csv(text, cols, N):
    ...
    if any(c not in df.columns for c in expect):
...
def maf_drift(realXf, syn_df):
    dr = []
    for c in realXf.columns:
        d = abs(maf(realXf[c].values) - maf(syn_df[c].values))
        dr.append(d)
    return float(np.mean(dr)), float(np.percentile(dr, 95))

def synth_yield(peppy, N, seed, target_corr=0.6):
    rng = np.random.default_rng(seed)
...
def main():
    ...
    args = ap.parse_args()
    ensure_dirs()
    cols, realXf = load_ipk()
    freqs = geno_freqs_ipk(realXf)
    prompt = build_prompt(cols, freqs, int(args.size))
    out, err, rc = run_ollama_cloud(prompt)
    cloud_status = 'OK' if rc == 0 else 'FAIL'
    if rc == 0:
        print('Ollama cloud model deepseek-v3.1:671b-cloud: OK')
    else:
        print('Ollama cloud model deepseek-v3.1:671b-cloud: FAIL', file=sys.stderr)
    try:
        syn_snps = parse_llm_csv(out, cols, int(args.size))
    except Exception:
        syn_snps = resample_freq(cols, freqs, int(args.size), args.seed)
    mean_d, p95_d = maf_drift(realXf, syn_snps)
    ...
    out_path = os.path.join(OUTDIR, 'synthetic_deepseek_ipk_200.csv')
    df.to_csv(out_path, index=False)
    pca_ratio = pca_capture(realXf, df.drop('Sample_ID', axis=1).drop('Yield_BV', axis=1))
    print(f'Corrélation Pearson Yield_BV (synthetic vs réel): {corr:.4f}')
    print(f'PCA capture ratio (synthetic at real threshold): {pca_ratio:.4f}')
    ...
    with open(os.path.join(OUTDIR, 'validation_log.txt'), 'w', encoding='utf-8') as f:
        f.write(f'deepseek-v3.1:671b-cloud status: {cloud_status}\n')
        f.write(f'Corrélation Pearson Yield_BV: {corr:.4f}\n')
        f.write(f'PCA capture ratio: {pca_ratio:.4f}\n')
        f.write(f'Moyenne réelle: {mu_r:.4f} | synthétique: {float(df["Yield_BV"].mean()):.4f}\n')
        f.write(f'Écart-type réel: {sd_r:.4f} | synthétique: {float(df["Yield_BV"].std(ddof=1)):.4f}\n')
        f.write(f'Plage réelle: [{lo:.4f}, {hi:.4f}]\n')
        f.write(f'Fichier: {out_path}\n')
