...
def train_eval_ridge(dfX: pd.DataFrame, dfy: pd.DataFrame, target_col: str, alphas: list[float]):
    ...
    for alpha in alphas:
        ...
        entry = {'alpha': alpha, 'r2_holdout': r2, 'rmse_holdout': rmse, 'cv_r2_mean': cv_r2, 'train_time_s': train_time}
        if best is None or entry['cv_r2_mean'] > best['cv_r2_mean']:
            best = entry
    best['n_samples'] = int(len(df))
    return best
...
def main():
    ...
    for name, Xdf, ydf in [
        ...
        res = train_eval_ridge(Xdf, ydf, 'Yield_BV', alphas)
        res['scenario'] = name
        results.append(res)
    df_res = pd.DataFrame(results)[['scenario','n_samples','alpha','r2_holdout','rmse_holdout','cv_r2_mean','train_time_s']]
    out_csv = os.path.join(OUTDIR, 'ridge_ipk_parquet_llms_results.csv')
    df_res.to_csv(out_csv, index=False)
    fig, ax = plt.subplots(figsize=(6,4))
    ax.bar(df_res['scenario'], df_res['r2_holdout'], color=['#4e79a7','#59a14f','#f28e2b','#e15759'])
    ax.set_ylabel('R² (holdout)')
    ax.set_title('Ridge R² (Parquet)')
    fig.tight_layout()
    fig_path_r2 = os.path.join(OUTDIR, 'ridge_ipk_r2_parquet.png')
    ...
    ax2.set_title('Ridge RMSE (Parquet)')
    fig2.tight_layout()
    fig_path_rmse = os.path.join(OUTDIR, 'ridge_ipk_rmse_parquet.png')
    fig2.savefig(fig_path_rmse)
    plt.close(fig2)
    # Temps entraînement baseline Parquet vs CSV et alpha optimal
    t_parq_start = time.time()
    _ = train_eval_ridge(scen_baseline_X, scen_baseline_y, 'Yield_BV', alphas)
    ...
    _csv = train_eval_ridge(X_csv, scen_baseline_y, 'Yield_BV', alphas)
    t_csv = time.time() - t_csv_start
    log_path = os.path.join(OUTDIR, 'ridge_training_time_log.txt')
    with open(log_path, 'w', encoding='utf-8') as f:
        f.write(f'baseline_parquet_train_time_s={t_parq:.4f}\n')
        f.write(f'baseline_csv_train_time_s={t_csv:.4f}\n')
        for r in results:
            f.write(f'scenario={r["scenario"]}, alpha_optimal={r["alpha"]}, cv_r2_mean={r["cv_r2_mean"]:.4f}\n')
