...
def main():
    ...
    out_csv = os.path.join(OUTDIR, 'lr_ipk_parquet_llms_results.csv')
    df_res.to_csv(out_csv, index=False)
    fig, ax = plt.subplots(figsize=(6,4))
    ax.bar(df_res['scenario'], df_res['r2_holdout'], color=['#4e79a7','#59a14f','#f28e2b','#e15759'])
    ax.set_ylabel('R² (holdout)')
    ax.set_title('Régression Linéaire R² (Parquet)')
    fig.tight_layout()
    fig_path_r2 = os.path.join(OUTDIR, 'lr_ipk_r2_parquet.png')
    ...
    ax2.set_title('Régression Linéaire RMSE (Parquet)')
    fig2.tight_layout()
    fig_path_rmse = os.path.join(OUTDIR, 'lr_ipk_rmse_parquet.png')
    ...
    log_path = os.path.join(OUTDIR, 'lr_training_time_log.txt')
