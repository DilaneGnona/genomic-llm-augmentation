...
PEP_X_PARQ = os.path.join(BASE, '02_processed_data', 'pepper', 'X.parquet')
PEP_X_CSV = os.path.join(BASE, '02_processed_data', 'pepper', 'X.csv')
PEP_Y = os.path.join(BASE, '02_processed_data', 'pepper', 'y.csv')
OUTDIR = os.path.join(BASE, '04_augmentation', 'pepper', 'glm46')
os.makedirs(OUTDIR, exist_ok=True)
N = 200
seed = 42

if os.path.exists(PEP_X_PARQ):
    X = pd.read_parquet(PEP_X_PARQ)
else:
    X = pd.read_csv(PEP_X_CSV)
...
out_path = os.path.join(OUTDIR, 'synthetic_glm46_pepper_200.csv')
