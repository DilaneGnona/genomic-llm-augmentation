"""
Context Learning with Kimi-K2.5 (Moonshot AI)
...
OUTDIR = os.path.join(BASE, '04_augmentation', 'pepper', 'model_sources', 'kimi')
...
def build_kimi_prompt(context_df, prompt_template, n_samples=50):
    """Build prompt for Kimi-K2.5"""
...
def call_kimi_api(prompt, api_key=None):
    """Call Kimi-K2.5 API via Moonshot AI"""
    try:
        ...
        if not api_key:
            ...
            return mock_kimi_response(prompt)
...
def parse_kimi_response(response_text, expected_cols):
    """Parse Kimi response into DataFrame"""
    try:
...
def log_run(context_name, prompt_name, n_samples, success, error_msg=None):
    """Log run to manifest"""
    manifest_path = os.path.join(LOGDIR, 'runs_manifest_kimi.csv')
...
def generate_with_kimi(context_file, prompt_file, n_samples=50):
    """Generate synthetic data using Kimi-K2.5"""
    print(f"\n{'='*60}")
    ...
    # Build prompt
    full_prompt, feature_cols = build_kimi_prompt(context_df, prompt_template, n_samples)
    
    # Call Kimi API
    print("Calling Kimi-K2.5 API...")
    t0 = time.time()
    response = call_kimi_api(full_prompt)
    elapsed = time.time() - t0
    print(f"Response received in {elapsed:.1f}s")
    
    # Parse response
    syn_df = parse_kimi_response(response, feature_cols)
    ...
    if syn_df is not None:
        ...
        # Save output
        output_file = f"synth_kimi_{prompt_file.replace('.txt', '')}_{context_file.replace('.csv', '')}.csv"
...
def main():
    ...
    print("="*60)
    print("Context Learning with Kimi-K2.5")
    print("="*60)
    ...
    for context_file, prompt_file, ctx_type in combinations:
        syn_df = generate_with_kimi(context_file, prompt_file, n_samples=50)
        results.append({
    ...
    print(f"Logs saved to: {LOGDIR}/runs_manifest_kimi.csv")
